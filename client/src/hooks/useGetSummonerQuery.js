...
const getSummoner = async (server, summonerName) => {
    const response = await fetch(
        `${process.env.REACT_APP_NOT_SECRET_CODE}/api/summoner/search_summoner/${server}/${summonerName.toLowerCase()}`
    );
...
export const useGetSummonerQuery = (server, summonerName) => {
    return useQuery(["summoner", server, summonerName.toLowerCase()], () => getSummoner(server, summonerName), {
        staleTime: 300000,
        retry: 1,
