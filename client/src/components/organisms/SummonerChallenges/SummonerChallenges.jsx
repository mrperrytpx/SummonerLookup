...
export const SummonerChallenges = () => {
    ...
    const { data: summonerChallengesData, isLoading } = useGetSummonerChallengesQuery(
        server,
        summonerName,
        summonerData.puuid
    );

    const sortedByFirst = useCallback(
        (index) => {
        ...
        <StyledSummonerChallenges>
            ...
            <div>
                {sortedChallenges.length ? (
                    ...
                    <div>
                        <ErrorText size="clamp(0.75rem, 2vw, 1.3rem)" center={true}>
                            No completed challenges for this category
                        </ErrorText>
                        <ErrorText size="clamp(0.75rem, 2vw, 1.3rem)" center={true}>
                            (┬┬﹏┬┬)
                        </ErrorText>
