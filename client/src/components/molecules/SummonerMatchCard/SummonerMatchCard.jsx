...
export const SummonerMatchCard = ({ match }) => {
    ...
    const gameVersion = match.gameVersion.split(".").slice(0, 2).join(".") + ".1";
                ...
                <div ref={matchRef} onClick={() => setIsExpanded(!isExpanded)}>
                    <FlexColCenter gap="0.25rem">
                        ...
                        <Span size="s">
                            {new Date(
                                match?.gameEndTimestamp
                                    ? match?.gameEndTimestamp
                                    : match?.gameStartTimestamp + match?.gameDuration
                            ).toDateString()}
                        </Span>
                    </FlexColCenter>
                    ...
                    <FlexColCenter gap="0.25rem">
                        <Span width="auto" align="center" size="s">
                            ({match.participants.kills}/{match.participants.deaths}/{match.participants.assists})
                        </Span>
                        <Span width="auto" align="center" size="s">
                            {match.participants.totalMinionsKilled} CS(
                            {Math.round((match.participants.totalMinionsKilled / match.gameDuration) * 600) / 10})
                        </Span>
                        <Span width="auto" align="center" size="s">
                    ...
                    <MatchItems gameVersion={gameVersion} items={itemsArray} initialWidth={35} />
                </div>
                {isExpanded && (
                    <ErrorBoundary>
