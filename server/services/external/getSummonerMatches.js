...
const getSummonerMatches = async (region, puuid, page) => {
    ...
    const matchesUrl = `https://${region}.api.riotgames.com/lol/match/v5/matches/by-puuid/${puuid}/ids?start=${(page - 1) * 10}&count=2&api_key=${process.env.RIOT_API}`;
    ...
    const hasNextPage = matchIds.length === 2;

    return [matchIds.slice(0, 2), hasNextPage];
};
