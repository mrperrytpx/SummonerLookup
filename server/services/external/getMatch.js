...
const getMatch = async (region, game) => {
    ...
    if (!gameResponse.ok) {
        if (gameResponse.status === 404)
            throw new ApiError("Match not found", 404, gameUrl);
        if (gameResponse.status === 429)
            throw new ApiError("Rate liit exceeded", 429, gameUrl);
    }
    ...
    if (+gameVersion[0] === 11 && +gameVersion[1] < 4)
        throw new Error("Game too old");
